...
// A call to `Intercept(f, g, h)` equals to `film.Intercept(f(g(h())))`.
func (c *FilmClient) Intercept(interceptors ...Interceptor) {
	c.inters.Film = append(c.inters.Film, interceptors...)
}

// Create returns a builder for creating a Film entity.
...
func (c *FilmClient) MapCreateBulk(slice any, setFunc func(*FilmCreate, int)) *FilmCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &FilmCreateBulk{err: fmt.Errorf("calling to FilmClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*FilmCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
...
func (c *FilmClient) UpdateOneID(id int64) *FilmUpdateOne {
...
func (c *FilmClient) DeleteOneID(id int64) *FilmDeleteOne {
...
func (c *FilmClient) Query() *FilmQuery {
	return &FilmQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeFilm},
		inters: c.Interceptors(),
	}
}

// Get returns a Film entity by its id.
func (c *FilmClient) Get(ctx context.Context, id int64) (*Film, error) {
	return c.Query().Where(film.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *FilmClient) GetX(ctx context.Context, id int64) *Film {
...
// QueryAuthor queries the author edge of a Film.
func (c *FilmClient) QueryAuthor(f *Film) *UserQuery {
...
// QueryManager queries the manager edge of a Film.
func (c *FilmClient) QueryManager(f *Film) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := f.ID
		step := sqlgraph.NewStep(
...
func (c *FilmClient) Interceptors() []Interceptor {
	return c.inters.Film
}

func (c *FilmClient) mutate(ctx context.Context, m *FilmMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&FilmCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&FilmUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&FilmUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&FilmDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown Film mutation op: %q", m.Op())
	}
}
